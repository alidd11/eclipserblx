import { MessageSquare, Package, Shield, Gift, Sparkles, Megaphone } from 'lucide-react';
...
export function AnnouncementsTab({ formData, handleChange, testingWebhook, webhookTestResults, onTest }: AnnouncementsTabProps) {
  return (
    <div className="space-y-4">
      ...
      <WebhookInput config={{ id: 'eclipse-plus', label: 'Eclipse+ Membership', description: 'Membership programme announcements', settingKey: 'eclipse_plus_discord_webhook_url', channelIdKey: 'eclipse_plus_discord_channel_id', icon: <Sparkles className="h-4 w-4 text-amber-400" />, iconColor: 'bg-amber-500/20' }} formData={formData} handleChange={handleChange} testingWebhook={testingWebhook} webhookTestResults={webhookTestResults} onTest={onTest} />
