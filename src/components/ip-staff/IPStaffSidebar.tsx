...
export function IPStaffSidebar({ collapsed, onToggle, onNavigate, isMobileDrawer = false, className }: IPStaffSidebarProps) {
  ...
  const renderNavLink = (item: NavItem) => {
    const isActive = location.pathname === item.href || 
      (item.href !== '/ip-staff' && location.pathname.startsWith(item.href));
        ...
        {!isCollapsed ? (
          ...
          <div className="relative px-1 pb-2 pt-4 flex flex-col items-center gap-2">
            <Tooltip>
              ...
              <TooltipContent side="right">IP Shield Staff</TooltipContent>
            </Tooltip>
          </div>
        )}
      </div>
