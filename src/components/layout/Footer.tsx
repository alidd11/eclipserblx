    ...
    <footer className="border-t border-border/40 mt-auto" role="contentinfo" aria-label="Site footer">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        ...
        <div className="sm:hidden grid grid-cols-2 gap-x-6 gap-y-6 mb-8">
          {columns.map((col) => (
            <div key={col.heading}>
              <p className="text-[10px] font-semibold uppercase tracking-widest text-foreground/40 mb-2.5">
                {col.heading}
              </p>
        ...
        <div className="hidden sm:grid grid-cols-4 gap-8 mb-8">
          {columns.map((col) => (
            <div key={col.heading} className="flex flex-col gap-1.5">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-foreground/40 mb-1.5" id={`footer-${col.heading.toLowerCase().replace(/\s/g, '-')}`}>
                {col.heading}
              </p>
              <nav className="flex flex-col gap-1.5" aria-labelledby={`footer-${col.heading.toLowerCase().replace(/\s/g, '-')}`}>
                {col.links.map((link) => (
                  <Link
                    key={link.href}
                    to={link.href}
                    className="text-[12px] text-foreground/50 hover:text-foreground transition-colors leading-relaxed"
                  >
                    {link.label}
                  </Link>
                ))}
        ...
        {/* ── Divider ── */}
        <div className="border-t border-border/30 mb-5 sm:mb-6" />
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <a href={discordUrl} target="_blank" rel="noopener noreferrer" aria-label="Discord" className="text-foreground/40 hover:text-foreground transition-colors">
              ...
              <a href="https://x.com/eclipserb" target="_blank" rel="noopener noreferrer" aria-label="X (Twitter)" className="text-foreground/40 hover:text-foreground transition-colors">
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              <span className="flex items-center gap-1 text-[10px] sm:text-[11px] text-foreground/40">
