...
// Apple Pay Logo - Official HIG style
function ApplePayMark() {
  return (
    <svg viewBox="0 0 50 21" className="h-6 w-auto">
      <path 
        fill="currentColor"
...
// Google Pay Logo - Official brand style  
...
export function PaymentRequestButton({ 
  ...
  }, [stripe, items, total, onWalletAvailable]);
  ...
  // Apple Pay - Official black button style
  if (walletType === 'applePay') {
    return (
      <button
        type="button"
        onClick={handleClick}
        className="w-full h-12 rounded-lg bg-foreground text-foreground font-medium flex items-center justify-center gap-2 transition-all hover:bg-foreground/90 active:scale-[0.98]"
  ...
  // Google Pay - Official white button style
  return (
    <button
      type="button"
      onClick={handleClick}
      className="w-full h-12 rounded-lg bg-background border border-border text-foreground font-medium flex items-center justify-center gap-2 transition-all hover:bg-gray-50 active:scale-[0.98]"
      style={{ WebkitTapHighlightColor: 'transparent' }}
    >
      <GooglePayMark />
    </button>
