      ...
      <div className="space-y-5 max-w-4xl mx-auto">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-xs sm:text-sm text-white/50 mt-1">Toggle commands on/off and re-register with Discord</p>
          </div>
          <Button
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            <Code className="h-10 w-10 mx-auto mb-3 text-white/20" />
            <p className="text-sm text-white/40">No commands configured</p>
          </div>
        ) : (
          <div className="space-y-2">
            {commands.map((cmd) => (
              <div key={cmd.id} className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl bg-white/5 border border-white/10 hover:bg-white/[0.07] transition-colors">
                <div className="flex-1 min-w-0">
                  <p className="font-mono text-sm text-white truncate">/{cmd.command_name}</p>
                  {cmd.description && (
