...
export default function BotCommunity() {
 ...
 <p className="text-white/50 text-sm mt-1">Giveaways, starboard, suggestions, verification, and more.</p>
 </div>
 ...
 <div className="px-4 py-3 border-b border-border bg-muted/30"><h3 className="font-semibold text-sm text-white text-base">Create Giveaway</h3></div>
 <div className="p-4 space-y-3">
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
 <div><Label className="text-white/60 text-xs">Title</Label><Input value={giveawayForm.title} onChange={e => setGiveawayForm({ ...giveawayForm, title: e.target.value })} className="bg-white/5 border-white/10 text-white" /></div>
 <div><Label className="text-white/60 text-xs">Prize</Label><Input value={giveawayForm.prize} onChange={e => setGiveawayForm({ ...giveawayForm, prize: e.target.value })} className="bg-white/5 border-white/10 text-white" /></div>
 <div><Label className="text-white/60 text-xs">Channel ID</Label><Input value={giveawayForm.channel_id} onChange={e => setGiveawayForm({ ...giveawayForm, channel_id: e.target.value })} className="bg-white/5 border-white/10 text-white" /></div>
 <div><Label className="text-white/60 text-xs">Winners</Label><Input type="number" min={1} value={giveawayForm.winners_count} onChange={e => setGiveawayForm({ ...giveawayForm, winners_count: parseInt(e.target.value) || 1 })} className="bg-white/5 border-white/10 text-white" /></div>
 <div><Label className="text-white/60 text-xs">Ends At</Label><Input type="datetime-local" value={giveawayForm.ends_at} onChange={e => setGiveawayForm({ ...giveawayForm, ends_at: e.target.value })} className="bg-white/5 border-white/10 text-white" /></div>
 </div>
 <Button onClick={createGiveaway} className="bg-[hsl(258,90%,66%)]"><Plus className="h-4 w-4 mr-2" /> Create</Button>
 </div>
 </div>
 <div className="border border-border rounded-xl overflow-hidden bg-white/[0.03] border-white/10">
 <div className="px-4 py-3 border-b border-border bg-muted/30"><h3 className="font-semibold text-sm text-white text-sm">Active Giveaways</h3></div>
 <div className="p-4 p-0">
 {giveaways.length === 0 ? (
 <div className="p-6 text-center text-white/40 text-sm">No giveaways yet.</div>
 ) : (
 ...
 <p className="text-xs text-white/40">Prize: {g.prize} · {g.winners_count} winner(s) · Ends {format(new Date(g.ends_at), 'dd MMM HH:mm')}</p>
 </div>
 ...
 <h3 className="font-semibold text-sm text-white text-base">Starboard</h3>
 <Switch checked={sbEnabled} onCheckedChange={setSbEnabled} />
 </div>
 </div>
 <div className="p-4 space-y-3">
 <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
 <div><Label className="text-white/60 text-xs">Channel ID</Label><Input value={sbChannel} onChange={e => setSbChannel(e.target.value)} className="bg-white/5 border-white/10 text-white" /></div>
 <div><Label className="text-white/60 text-xs">Emoji</Label><Input value={sbEmoji} onChange={e => setSbEmoji(e.target.value)} className="bg-white/5 border-white/10 text-white" /></div>
 <div><Label className="text-white/60 text-xs">Threshold</Label><Input type="number" min={1} value={sbThreshold} onChange={e => setSbThreshold(parseInt(e.target.value) || 3)} className="bg-white/5 border-white/10 text-white" /></div>
 </div>
 ...
 <div className="border border-border rounded-xl overflow-hidden bg-white/[0.03] border-white/10">
 <div className="px-4 py-3 border-b border-border bg-muted/30"><h3 className="font-semibold text-sm text-white text-base">Suggestion Settings</h3></div>
 <div className="p-4 space-y-3">
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
 <div><Label className="text-white/60 text-xs">Suggestions Channel ID</Label><Input defaultValue={suggestionsConfig?.channel_id || ''} id="sug-channel" className="bg-white/5 border-white/10 text-white" /></div>
 <div><Label className="text-white/60 text-xs">Review Channel ID</Label><Input defaultValue={suggestionsConfig?.review_channel_id || ''} id="sug-review" className="bg-white/5 border-white/10 text-white" /></div>
 </div>
 <Button onClick={() => {
 ...
 <div className="border border-border rounded-xl overflow-hidden bg-white/[0.03] border-white/10">
 <div className="px-4 py-3 border-b border-border bg-muted/30"><h3 className="font-semibold text-sm text-white text-sm">Recent Suggestions</h3><p className="text-sm text-muted-foreground text-white/40">{suggestions.length} total</p></div>
 <div className="p-4 p-0">
 {suggestions.length === 0 ? (
 <div className="p-6 text-center text-white/40 text-sm">No suggestions yet. Members use <code>/suggest</code>.</div>
 ) : (
 <div className="divide-y divide-white/5">
 {suggestions.map((s: any) => (
 <div key={s.id} className="px-4 py-3">
 <div className="flex items-center gap-2">
 <span className="text-sm text-white">{s.content}</span>
 ...
 <p className="text-xs text-white/40 mt-0.5">by {s.author_username || s.author_id} · 👍 {s.upvotes} 👎 {s.downvotes}</p>
 </div>
 ))}
 </div>
 )}
 ...
 <div className="px-4 py-3 border-b border-border bg-muted/30">
 <div className="flex items-center justify-between">
 <h3 className="font-semibold text-sm text-white text-base">Verification Gate</h3>
 <Switch checked={vEnabled} onCheckedChange={setVEnabled} />
 </div>
 <p className="text-sm text-muted-foreground text-white/40">Require users to verify before accessing the server.</p>
 </div>
 <div className="p-4 space-y-3">
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
 <div><Label className="text-white/60 text-xs">Verification Channel ID</Label><Input value={vChannel} onChange={e => setVChannel(e.target.value)} className="bg-white/5 border-white/10 text-white" /></div>
 <div><Label className="text-white/60 text-xs">Verified Role ID</Label><Input value={vRole} onChange={e => setVRole(e.target.value)} className="bg-white/5 border-white/10 text-white" /></div>
 </div>
 ...
 <div className="px-4 py-3 border-b border-border bg-muted/30">
 <div className="flex items-center justify-between">
 <h3 className="font-semibold text-sm text-white text-base">Join Gate Filters</h3>
 <Switch checked={jgEnabled} onCheckedChange={setJgEnabled} />
 </div>
 <p className="text-sm text-muted-foreground text-white/40">Filter new joins by account age, avatar, etc.</p>
 </div>
 <div className="p-4 space-y-3">
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
 <div><Label className="text-white/60 text-xs">Min Account Age (days)</Label><Input type="number" min={0} value={jgMinAge} onChange={e => setJgMinAge(parseInt(e.target.value) || 0)} className="bg-white/5 border-white/10 text-white" /></div>
 <div className="flex items-center gap-3 pt-5">
 <Switch checked={jgAvatar} onCheckedChange={setJgAvatar} />
 <Label className="text-white/60 text-xs">Require Avatar</Label>
 </div>
 </div>
 ...
 <div className="border border-border rounded-xl overflow-hidden bg-white/[0.03] border-white/10">
 <div className="px-4 py-3 border-b border-border bg-muted/30"><h3 className="font-semibold text-sm text-white text-base">Scheduled Messages</h3></div>
 <div className="p-4 p-0">
 {scheduledMsgs.length === 0 ? (
 <div className="p-6 text-center text-white/40 text-sm">No scheduled messages. Create one from the bot commands.</div>
 ) : (
 <div className="divide-y divide-white/5">
 {scheduledMsgs.map((m: any) => (
 <div key={m.id} className="px-4 py-3">
 <p className="text-sm text-white truncate">{m.content || 'Embed message'}</p>
 <p className="text-xs text-white/40">Channel: {m.channel_id} · {m.repeat ? 'Repeating' : 'One-time'} · {m.enabled ? 'Active' : 'Disabled'}</p>
 </div>
 ))}
 </div>
 )}
 ...
 <div className="border border-border rounded-xl overflow-hidden bg-white/[0.03] border-white/10">
 <div className="px-4 py-3 border-b border-border bg-muted/30"><h3 className="font-semibold text-sm text-white text-base">Auto-Role on Join</h3></div>
 <div className="p-4 space-y-3">
 <div className="flex gap-2">
 <Input value={newAutoRole.role_id} onChange={e => setNewAutoRole({ ...newAutoRole, role_id: e.target.value })} placeholder="Role ID" className="bg-white/5 border-white/10 text-white flex-1" />
 <Input value={newAutoRole.role_name} onChange={e => setNewAutoRole({ ...newAutoRole, role_name: e.target.value })} placeholder="Role name" className="bg-white/5 border-white/10 text-white flex-1" />
 <Button onClick={() => {
 ...
 {autoRoles.map((r: any) => (
 <div key={r.id} className="flex items-center justify-between bg-white/5 rounded px-3 py-2">
 <span className="text-sm text-white">{r.role_name || r.role_id}</span>
 ...
 <div className="border border-border rounded-xl overflow-hidden bg-white/[0.03] border-white/10">
 <div className="px-4 py-3 border-b border-border bg-muted/30"><h3 className="font-semibold text-sm text-white text-base">Join/Leave Announcements</h3></div>
 <div className="p-4 space-y-4">
 <div className="space-y-3">
 <div className="flex items-center gap-3"><Switch checked={jlJoinEnabled} onCheckedChange={setJlJoinEnabled} /><Label className="text-white/60 text-sm">Enable Join Messages</Label></div>
 {jlJoinEnabled && (
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pl-6">
 <div><Label className="text-white/60 text-xs">Channel ID</Label><Input value={jlJoinChannel} onChange={e => setJlJoinChannel(e.target.value)} className="bg-white/5 border-white/10 text-white" /></div>
 <div><Label className="text-white/60 text-xs">Message</Label><Input value={jlJoinMsg} onChange={e => setJlJoinMsg(e.target.value)} className="bg-white/5 border-white/10 text-white" /></div>
 </div>
 )}
 </div>
 <div className="space-y-3">
 <div className="flex items-center gap-3"><Switch checked={jlLeaveEnabled} onCheckedChange={setJlLeaveEnabled} /><Label className="text-white/60 text-sm">Enable Leave Messages</Label></div>
 {jlLeaveEnabled && (
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pl-6">
 <div><Label className="text-white/60 text-xs">Channel ID</Label><Input value={jlLeaveChannel} onChange={e => setJlLeaveChannel(e.target.value)} className="bg-white/5 border-white/10 text-white" /></div>
 <div><Label className="text-white/60 text-xs">Message</Label><Input value={jlLeaveMsg} onChange={e => setJlLeaveMsg(e.target.value)} className="bg-white/5 border-white/10 text-white" /></div>
 </div>
 )}
 </div>
 ...
 <div className="px-4 py-3 border-b border-border bg-muted/30">
 <div className="flex items-center justify-between">
 <h3 className="font-semibold text-sm text-white text-base">Mod Log Channel</h3>
 <Switch checked={mlEnabled} onCheckedChange={setMlEnabled} />
 </div>
 <p className="text-sm text-muted-foreground text-white/40">Log moderation actions to a Discord channel.</p>
 </div>
 <div className="p-4 space-y-3">
 <div><Label className="text-white/60 text-xs">Log Channel ID</Label><Input value={mlChannel} onChange={e => setMlChannel(e.target.value)} className="bg-white/5 border-white/10 text-white" /></div>
