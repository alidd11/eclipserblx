    ...
    <BotDashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-foreground/50 text-sm mt-1">Bot error logs and stack traces for debugging.</p>
        </div>
        ...
        <div className="border border-white/10 rounded-xl overflow-hidden bg-background/[0.03]">
          <div className="px-6 py-4 bg-background/[0.03] border-b border-white/10">
            <h3 className="text-foreground text-base font-semibold">Recent Errors</h3>
            <p className="text-foreground/40 text-sm">{filtered.length} log entries</p>
          </div>
          <div className="p-0">
            {isLoading ? (
                    ...
                    <div key={log.id} className="hover:bg-background/[0.02]">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <Badge variant="outline" className="text-[10px] border-white/20 text-foreground/50">{log.context}</Badge>
                            <span className="text-[11px] text-foreground/30">
                              {format(new Date(log.created_at), 'dd MMM HH:mm')}
                            </span>
                          </div>
                          <p className="text-sm text-foreground/70 mt-1 truncate">{log.error_message}</p>
                        </div>
                        {isExpanded ? (
                      ...
                      {isExpanded && log.stack_trace && (
                        <div className="px-4 pb-3">
                          <pre className="text-[11px] text-foreground/40 bg-foreground/30 rounded-lg p-3 overflow-x-auto whitespace-pre-wrap">
                            {log.stack_trace}
                          </pre>
                        </div>
                      )}
