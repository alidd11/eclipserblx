...
async function handleDailyCommand(
  ...
  await supabase
    ...
    .update({ 
      ...
      longest_streak: supabase.rpc ? currentStreak : currentStreak,
    })
    .eq("discord_id", discordUserId);
  ...
  const embed = {
    ...
    footer: { text: "Eclipse XP System • Claim again tomorrow!" },
    timestamp: new Date().toISOString(),
  };
  ...
  if (!xpData) {
    const embed = {
      ...
      description: targetUser 
        ...
        : "You haven't earned any XP yet!\n\nUse `/daily` to claim your first daily reward, or interact with the community!",
      thumbnail: discordAvatarUrl ? { url: discordAvatarUrl } : undefined,
      footer: { text: "Eclipse XP System" },
    };
...
async function handleLeaderboardCommand(supabase: any, discordAvatarUrl?: string) {
  ...
  const embed = {
    ...
    footer: { text: "Eclipse XP System • Top 10 Users" },
    timestamp: new Date().toISOString(),
  };
...
async function handleStreakCommand(
  ...
  const embed = {
    ...
    footer: { text: `${discordUsername} • Eclipse XP System` },
    timestamp: new Date().toISOString(),
  };
...
function handleJokeCommand(discordAvatarUrl?: string) {
  ...
  const embed = {
    ...
    footer: { text: "Eclipse Fun Commands" },
    timestamp: new Date().toISOString(),
  };
...
function handleFunFactCommand(discordAvatarUrl?: string) {
  ...
  const embed = {
    ...
    title: "🧠 Fun Fact",
    description: fact,
    footer: { text: "Eclipse Fun Commands • Did you know?" },
    timestamp: new Date().toISOString(),
  };
...
function handleComplimentCommand(
  ...
  const embed = {
    ...
    thumbnail: discordAvatarUrl ? { url: discordAvatarUrl } : undefined,
    footer: { text: targetUser ? `From ${discordUsername}` : "Eclipse Fun Commands" },
    timestamp: new Date().toISOString(),
  };
