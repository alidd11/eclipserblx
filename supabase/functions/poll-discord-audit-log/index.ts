...
async function supabaseSelect(table: string, params: string): Promise<unknown[]> {
...
async function supabaseUpsert(table: string, body: Record<string, unknown>): Promise<void> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`DB upsert error: ${res.status} ${t}`);
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    // Get the last processed audit log ID
    const cursorRows = await supabaseSelect(
      "discord_audit_log_cursor",
      `guild_id=eq.${guildId}&select=last_audit_log_id&limit=1`
    ) as Array<{ last_audit_log_id: string | null }>;

    const lastProcessedId = cursorRows?.[0]?.last_audit_log_id ?? null;
    ...
    if (lastProcessedId) {
      ...
      if (cutoffIndex === 0) {
        ...
        await supabaseUpsert("discord_audit_log_cursor", {
          guild_id: guildId,
          last_polled_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
          last_audit_log_id: lastProcessedId,
        });
        return new Response(JSON.stringify({ processed: 0 }), {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      if (cutoffIndex > 0) newEntries = entries.slice(0, cutoffIndex);
    }
    ...
    for (const entry of newEntries) {
      try {
        ...
        switch (entry.action_type) {
          case MEMBER_KICK: await handleKick(entry, performer, target); processed++; break;
          case MEMBER_BAN_ADD: await handleBan(entry, performer, target); processed++; break;
          case MEMBER_ROLE_UPDATE: await handleRoleUpdate(entry, performer, target); processed++; break;
          case MESSAGE_DELETE: await handleMessageDelete(entry, performer, target); processed++; break;
        }
      } catch (entryError) {
...
async function handleBan(entry: AuditLogEntry, performer?: DiscordUser, target?: DiscordUser) {
  ...
  if (performer) fields.push({ name: "Banned By", value: `👤 <@${performer.id}>`, inline: true });
  if (entry.reason) fields.push({ name: "Reason", value: entry.reason, inline: false });
  ...
  for (const change of entry.changes) {
    if (change.key === "$add" && Array.isArray(change.new_value)) {
      for (const role of change.new_value as Array<{ id: string; name: string }>) {
        ...
        if (performer) fields.push({ name: "Performed By", value: `👤 <@${performer.id}>`, inline: true });
        await sendBotMessage(ROLE_LOG_CHANNEL_ID, {
          embeds: [{
            author: { name: targetName, icon_url: getAvatarUrl(target) },
            description: `<@${targetId}> was given the <@&${role.id}> role.`,
            color: ROLE_ADD_COLOR, fields,
            timestamp: new Date().toISOString(),
          }],
        });
      }
      ...
      for (const role of change.new_value as Array<{ id: string; name: string }>) {
        ...
        await sendBotMessage(ROLE_LOG_CHANNEL_ID, {
          embeds: [{
            ...
            color: ROLE_REMOVE_COLOR, fields,
            timestamp: new Date().toISOString(),
          }],
        });
      }
...
async function handleMessageDelete(entry: AuditLogEntry, performer?: DiscordUser, target?: DiscordUser) {
  ...
  if (performer && performer.id !== targetId) {
    fields.push({ name: "Deleted By", value: `👤 <@${performer.id}>`, inline: true });
  }
