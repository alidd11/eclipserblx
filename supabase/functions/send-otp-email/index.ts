...
// Generate Eclipse branded HTML email template with Purple & Obsidian theme
function generateOtpEmailHtml(token: string, email_action_type: string): string {
  ...
  const getIcon = () => {
    switch (email_action_type) {
      case 'signup':
        return `<path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" fill="#a855f7" stroke="#a855f7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>`
      case 'recovery':
        return `<rect x="3" y="11" width="18" height="11" rx="2" ry="2" fill="none" stroke="#a855f7" stroke-width="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4" fill="none" stroke="#a855f7" stroke-width="2"/>`
      default:
        return `<path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" fill="none" stroke="#a855f7" stroke-width="2" stroke-linecap="round"/><polyline points="22 4 12 14.01 9 11.01" fill="none" stroke="#a855f7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>`
    }
  }
        ...
        <table role="presentation" width="520" cellspacing="0" cellpadding="0" style="max-width: 520px; background: linear-gradient(180deg, #151518 0%, #0d0d10 100%); border: 1px solid rgba(168, 85, 247, 0.2); border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td align="center" style="padding: 0 40px 12px;">
              <h1 style="font-size: 26px; font-weight: 700; color: #ffffff; margin: 0; font-family: Georgia, serif;">${getHeaderText()}</h1>
            </td>
          </tr>
          ...
          <tr>
            <td align="center" style="padding: 0 40px 24px;">
              <table role="presentation" cellspacing="0" cellpadding="0" style="background: linear-gradient(135deg, #1a1520 0%, #12101a 100%); border: 2px solid #a855f7; border-radius: 12px; box-shadow: 0 0 30px rgba(168, 85, 247, 0.2), inset 0 1px 0 rgba(255,255,255,0.05);">
                <tr>
                  <td style="padding: 20px 36px;">
                    <span style="font-size: 38px; font-weight: 800; color: #a855f7; letter-spacing: 12px; font-family: 'SF Mono', Monaco, 'Roboto Mono', Consolas, monospace; text-shadow: 0 0 20px rgba(168, 85, 247, 0.5);">${formattedToken}</span>
                  </td>
                </tr>
              </table>
            </td>
