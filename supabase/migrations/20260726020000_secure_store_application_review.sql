...
-- The original UNIQUE(user_id, status) constraint prevented a seller from
-- re-applying after more than one rejection. Only concurrent pending
-- applications need to be prohibited.
...
-- Applicants may only create a pending, unreviewed application. In particular,
-- a client must not be able to submit status='approved'.
...
-- Reading the administrative queue is permission-scoped. Applicants retain
-- access to their own rows via the existing owner policy.
...
-- Review decisions must go through the atomic RPC below. Direct UPDATE could
-- otherwise produce an "approved" application without activating a store,
-- notifying the applicant, or writing an audit log.
...
-- Store creation is exclusively part of the atomic review RPC. Allowing an
-- applicant to create even an inactive pending store here would let one
-- application create multiple orphan store records outside the review lock.
...
-- Production still has the legacy three-argument overload. CREATE OR REPLACE
-- with the new notes argument would otherwise leave that older RPC callable.
...
BEGIN
  ...
  -- Repeat submissions of the same decision are safe and do not create a
  -- second store or duplicate audit/notification records.
  ...
  IF p_decision = 'approved' THEN
    ...
    -- Reuse a pre-existing store for the owner rather than creating duplicate
    -- store/balance records in the event of older administrative workflows.
    SELECT *
    INTO v_store
    FROM public.stores
    ...
    IF NOT FOUND THEN
      ...
      VALUES (
        ...
        v_application.store_name,
        v_slug,
        v_application.store_description,
        v_application.discord_server_invite,
        'approved',
        true,
        false,
        v_reviewer_id,
