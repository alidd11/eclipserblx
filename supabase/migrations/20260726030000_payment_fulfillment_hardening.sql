...
-- Existing rows were written only by the legacy "already processed" path.
...
BEGIN
  ...
  -- A currently running delivery owns the event for five minutes. Failed or
  -- stale deliveries are reclaimable so Stripe retries can repair fulfilment.
