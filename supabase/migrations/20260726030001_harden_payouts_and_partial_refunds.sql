-- Make payout reservations/finalization and cumulative refund accounting
-- transaction-safe and retry-safe.
...
-- Payouts must go through the transaction-safe RPCs. Direct INSERT policies
-- allowed clients to create unreserved payout rows and bypass ownership checks.
...
-- This is the sole seller-facing creation path after the direct INSERT policy
-- is removed. Validate ownership and numeric input, serialize on the balance
-- row, and prevent more than one open payout per store.
...
BEGIN
  ...
  SELECT COALESCE(
    (
      ...
      WHERE key = 'seller_minimum_payout'
      LIMIT 1
    ),
    5
  )
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'No seller balance found';
  END IF;

  IF p_amount < v_min_payout THEN
    RAISE EXCEPTION 'Payout amount is below the minimum';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE store_id = p_store_id
      AND status IN ('pending', 'processing', 'awaiting_funds')
  ) THEN
    RAISE EXCEPTION 'A seller payout is already pending';
  END IF;

  SELECT payout_method, paypal_email
...
REVOKE ALL ON FUNCTION public.request_seller_payout(uuid, uuid, numeric)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.request_seller_payout(uuid, uuid, numeric)
  TO authenticated;

-- These legacy SECURITY DEFINER helpers were created before execute privileges
-- were locked down. They are backend orchestration primitives, not public RPCs.
REVOKE ALL ON FUNCTION public.claim_payout_for_processing(uuid, text, text)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.deduct_seller_balance(uuid, numeric)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.claim_payout_for_processing(uuid, text, text)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.deduct_seller_balance(uuid, numeric)
  TO service_role;

UPDATE public.seller_transactions
SET refunded_amount = COALESCE(net_amount, 0)
WHERE refunded_at IS NOT NULL
  AND refunded_amount = 0;

UPDATE public.affiliate_commissions
SET reversed_amount = COALESCE(commission_amount, 0)
WHERE reversed_at IS NOT NULL
  AND reversed_amount = 0;

-- Release only the unrefunded portion of an escrowed sale. The legacy function
-- moved the original net amount even after a partial refund and skipped fully
-- refunded transactions without removing their pending balance.
...
BEGIN
  ...
  LOOP
    ...
    WHERE user_id = v_tx.seller_id
      AND COALESCE(pending_balance, 0) >= v_remaining;

    IF NOT FOUND THEN
      RAISE EXCEPTION 'Seller balance not found during escrow release';
    END IF;
...
-- Existing payout requests already reserve the affiliate's available balance in
-- the Edge Function, so mark open requests as reserved before replacing the
-- legacy completion trigger.
UPDATE public.affiliate_payouts
SET balance_reserved_at = COALESCE(balance_reserved_at, created_at, now())
  ...
  IF NEW.status = 'completed' AND OLD.status IS DISTINCT FROM 'completed' THEN
    ...
    SET total_paid = total_paid + NEW.amount,
        available_balance = CASE
          WHEN NEW.balance_reserved_at IS NULL
            THEN GREATEST(0, available_balance - NEW.amount)
          ELSE available_balance
        END,
        updated_at = now()
    WHERE user_id = NEW.user_id;
  ELSIF NEW.status IN ('failed', 'rejected')
    AND OLD.status IS DISTINCT FROM NEW.status
    AND NEW.balance_reserved_at IS NOT NULL
    AND NEW.balance_released_at IS NULL
  THEN
    UPDATE public.affiliate_balances
...
BEGIN
  ...
  -- Serialize reservations per affiliate even when no prior payout row exists.
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(p_user_id::text, 0)
  );

  -- A retry of the same request reuses its reservation and provider
  -- idempotency key instead of deducting the balance twice.
  SELECT *
  INTO v_existing
  ...
  IF FOUND THEN
    IF v_existing.amount = p_amount
      AND COALESCE(v_existing.payout_method, 'paypal') = p_payout_method
    THEN
  ...
  VALUES (
    ...
    p_paypal_email,
    p_notes,
    CASE WHEN p_payout_method = 'stripe' THEN 'processing' ELSE 'pending' END,
    now()
  )
  RETURNING id INTO v_payout_id;
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Affiliate payout not found';
  END IF;

  IF v_payout.status = 'completed' THEN
    RETURN v_payout.stripe_transfer_id = p_stripe_transfer_id;
  END IF;
  ...
  SET status = 'completed',
      ...
      processed_at = now(),
      notes = 'Automatic Stripe transfer: ' || p_stripe_transfer_id
  WHERE id = p_payout_id;
...
BEGIN
  ...
  UPDATE public.affiliate_payouts
  SET status = 'failed',
      notes = LEFT(COALESCE(p_failure_reason, 'Payout provider request failed'), 500)
  WHERE id = p_payout_id;
...
CREATE OR REPLACE FUNCTION public.finalize_seller_payout(
  ...
  p_status text,
  p_notes text,
  p_provider_secondary_reference text DEFAULT NULL
)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF p_provider NOT IN ('stripe', 'wise', 'paypal')
    ...
    OR NULLIF(p_provider_reference, '') IS NULL
  THEN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Seller payout not found';
  END IF;

  IF (v_payout.status = p_status OR v_payout.status = 'completed')
    AND (
      (p_provider = 'stripe' AND v_payout.stripe_transfer_id = p_provider_reference)
  ...
  -- A processing provider response (for example, a funded Wise transfer) is
  -- not a completed payout. Keep the seller's funds reserved in pending until
  -- a signed provider webhook reports a terminal state.
  IF p_status = 'completed' THEN
    ...
    WHERE user_id = v_payout.seller_id
      AND pending_balance >= v_payout.amount;

    IF NOT FOUND THEN
      RAISE EXCEPTION 'Insufficient seller balance during payout finalization';
    END IF;
  END IF;
  ...
  SET status = p_status,
      ...
      stripe_transfer_id = CASE
        WHEN p_provider = 'stripe' THEN p_provider_reference
        ELSE stripe_transfer_id
      END,
      wise_transfer_id = CASE
        WHEN p_provider = 'wise' THEN p_provider_reference
        ELSE wise_transfer_id
      END,
      wise_quote_id = CASE
        WHEN p_provider = 'wise' THEN p_provider_secondary_reference
        ELSE wise_quote_id
      END,
      funding_status = CASE
        WHEN p_provider = 'wise' THEN 'funded'
        ELSE funding_status
      END,
...
BEGIN
  ...
  FROM public.seller_payouts
  WHERE id = p_payout_id
    AND wise_transfer_id = p_provider_reference
  FOR UPDATE;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Insufficient seller balance during payout settlement';
  END IF;
  ...
  SET status = p_status,
      ...
      failure_reason = CASE
        WHEN p_status = 'failed'
          THEN LEFT(COALESCE(p_failure_reason, 'Payout provider reported failure'), 1000)
        ELSE NULL
      END,
      processing_locked_at = NULL,
      processing_lock_id = NULL
...
BEGIN
  IF NULLIF(p_charge_id, '') IS NULL
    OR p_cumulative_amount IS NULL
    OR p_cumulative_amount < 0
    OR p_cumulative_amount = 'NaN'::numeric
  THEN
  ...
  LOOP
    v_target := CASE
      WHEN p_is_full_refund OR v_cumulative >= v_order.total
        THEN COALESCE(v_transaction.net_amount, 0)
      ELSE ROUND(COALESCE(v_transaction.net_amount, 0) * v_fraction, 2)
    END;
    ...
    IF v_adjustment > 0 THEN
      ...
      SET total_earned = GREATEST(0, total_earned - v_adjustment),
          pending_balance = CASE
            WHEN v_transaction.escrow_released_at IS NULL
              THEN GREATEST(0, COALESCE(pending_balance, 0) - v_adjustment)
            ELSE pending_balance
          END,
          available_balance = CASE
            WHEN v_transaction.escrow_released_at IS NOT NULL
              THEN GREATEST(0, COALESCE(available_balance, 0) - v_adjustment)
            ELSE available_balance
          END,
          updated_at = v_now
      ...
      IF NOT FOUND THEN
        RAISE EXCEPTION 'Seller balance not found during refund accounting';
      END IF;

      UPDATE public.seller_transactions
      SET refunded_amount = v_target,
          refunded_at = CASE
            WHEN v_target >= COALESCE(net_amount, 0) THEN v_now
            ELSE refunded_at
          END,
  ...
  LOOP
    v_target := CASE
      WHEN p_is_full_refund OR v_cumulative >= v_order.total
        THEN COALESCE(v_commission.commission_amount, 0)
      ELSE ROUND(COALESCE(v_commission.commission_amount, 0) * v_fraction, 0)
    END;
    ...
    IF v_adjustment > 0 THEN
      ...
      SET reversed_amount = v_target,
          status = CASE
            WHEN v_target >= COALESCE(commission_amount, 0) THEN 'reversed'
            ELSE status
          END,
          reversed_at = CASE
            WHEN v_target >= COALESCE(commission_amount, 0) THEN v_now
            ELSE reversed_at
          END,
          refund_id = p_charge_id
  ...
  SET status = CASE
        WHEN p_is_full_refund OR v_cumulative >= total THEN 'refunded'
        ELSE 'partially_refunded'
      END,
      refunded_at = CASE
        WHEN p_is_full_refund OR v_cumulative >= total THEN v_now
        ELSE refunded_at
      END,
      refund_amount = v_cumulative,
      refund_id = p_charge_id
  WHERE id = p_order_id;
